$(function(){
    ...
    $(".tab a").on("click", function () {
        ...
        $(this).addClass("on");
        $(".tab_cts").removeClass("on");
        $(".tab_cts").eq(idx).addClass("on");
    });
    /* //탭*/
    ...
    /*팝업*/
    $(".layer_open").on("click", function (ele) {
        var open= $(this);
/*        var ev = ev || window.event;
        ...
        var layer = "#"+$target
        $(layer).focus();
        // $(".ex_03 a").attr('tabindex','0').show().focus();
        ...
        $(layer).find('.cls').on("click", function () {
            $(layer).hide();
            if(open.attr('class')=='layer_open inn_pop'){
                return false;
            }else{
               $(".dim").hide(); 
            }
    ...
    $(".ex_08 .auto").on("click", function () {
        autoPlay = setInterval(function() {
            autoSlide();
        }, 2000);

    });
    $(".ex_08 .stop").on("click", function () {
       clearInterval(autoPlay)
    });

    function moveSlide(nextIndex) {
        if($(".ex_08 .slide li").is(':animated')){
            return false
        }
...
nsy_ui.aniEff = function () {
    ...
    $('.ani_eff').find('p').eq(0).css({
        top: 0,
        color: 'red'
    })
    $('.ani_eff').find('p').eq(1).css({
        top: 35,
        color: 'blue'
    })
    $('.ani_eff').find('p').eq(0).delay(100).animate({
        left: 300,
        opacity: 1
    }, 1000)
    $('.ani_eff').find('p').eq(1).delay(200).animate({
        left: 390,
        opacity: 1
    }, 1000)
}
